...
class RuleConfig:
    ...
    description: str
    enabled: bool = True
    # Aquí se modifican números/variables de la regla (umbrales, ventanas, etc.). / This is where rule numbers/variables are modified (thresholds, windows, etc.).
...
def build_rule_parameters_template(
    *, threshold: str = "", window: str = "", notes: str = ""
    ...
    # Nota: estos campos son los puntos donde se cambian números/variables. / Note: these fields are the points where numbers/variables are changed.
    return {
        "threshold": threshold,
        "window": window,
        "notes": notes,
