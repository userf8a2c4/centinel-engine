...
| RT-12 | Bypass potencial por reuso de conexiones persistentes (pooling HTTP) en flujo de validación DNS | ✅ Resuelto | El collector crítico usa transporte HTTPS pinneado a IP validada (sin pool compartido de `requests`) + `Host` explícito + `Connection: close`. | Se elimina reuso de socket persistente y se reduce la ventana de TOCTOU en la ruta auditada. | Mantener transporte pinneado por IP en rutas críticas y ampliar el patrón al resto de canales salientes. |
| RT-13 | Allowlist de backup remoto opcional (fail-open operacional) | ✅ Resuelto | Provider `github` opera en fail-closed y valida el **URL real del remoto** (`git remote get-url`) contra `BACKUP_GIT_REMOTE_ALLOWLIST`; no confía en alias como `origin`. | Se bloquea exfiltración por alias remoto apuntando a URL no permitida. | Mantener allowlist por URL canónica y auditar cambios de remotos en runtime/CI. |
| RT-14 | Coherencia de anonimización aún débil por derivación determinística basada en path | ✅ Resuelto | El salt ahora se obtiene desde secreto de entorno robusto o desde secreto local persistido; además se rechazan `ATTACK_LOG_SALT` triviales/cortos para evitar pseudonimización débil. | Se evita correlación predecible entre despliegues por salts de baja entropía. | Rotar salt con procedimiento controlado y versionado de ventanas analíticas. |
| RT-15 | Límite de `air_gap` no persiste entre reinicios (bypass por restart) | ✅ Resuelto | `last_air_gap_at` se restaura/persiste en `deadman_state_path`, manteniendo cooldown tras restart. | Reinicios no resetean el límite temporal del air-gap en el gestor defensivo. | Vigilar integridad/permisos del archivo de estado durable. |
...
- Prioridad recomendada inmediata: seguimiento continuo de regresiones para RT-11..RT-15 (cerrados en esta iteración de hardening).
