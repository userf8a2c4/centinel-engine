...
class InconsistentActsTracker:
    ...
    def __init__(
        ...
        *,
        ...
        low_delta_percentile_range: tuple[float, float] = (0.05, 0.40),
        high_inconsistent_percentile_range: tuple[float, float] = (0.60, 1.0),
        min_baseline_cycles: int = 8,
        low_variance_ratio_threshold: float = 0.15,
    ) -> None:
        ...
        self.adaptive_thresholds_enabled = bool(runtime_config.get("adaptive_thresholds_enabled", adaptive_thresholds_enabled))
        self.low_delta_percentile_range = self._parse_range(
            runtime_config.get("low_delta_percentile_range", low_delta_percentile_range),
            low_delta_percentile_range,
        )
        self.high_inconsistent_percentile_range = self._parse_range(
            runtime_config.get("high_inconsistent_percentile_range", high_inconsistent_percentile_range),
            high_inconsistent_percentile_range,
        )
        self.min_baseline_cycles = int(runtime_config.get("min_baseline_cycles", min_baseline_cycles))
        self.low_variance_ratio_threshold = float(
            runtime_config.get("low_variance_ratio_threshold", low_variance_ratio_threshold)
        )
    ...
    def load_snapshot(self, json_data: dict, timestamp: datetime) -> None:
        ...
        low_delta_min, low_delta_max, high_inconsistent_min = self._effective_injection_thresholds()
        if high_inconsistent_min <= inconsistent_count and low_delta_min <= delta_total <= low_delta_max:
    ...
    def detect_progressive_injection(self) -> dict[str, Any] | None:
        ...
        detection = {
            ...
            "description": (
                ...
                f"{len(self.injection_history)} ciclos consecutivos con delta en rango [{low_delta_min:.1f}, {low_delta_max:.1f}] "
                f"mientras AI >= {high_inconsistent_min:.1f}"
            ),
        }
    ...
    def run_injection_statistical_tests(self, deltas: list[float]) -> dict[str, float | bool]:
        ...
        baseline_ratio = deltas_variance / max(baseline_var, 1e-9)
        improbable = bool(
            z_pvalue < 0.01
            or run_pvalue < self.run_test_pvalue_threshold
            or (len(deltas) >= self.min_consecutive_injections and baseline_ratio < self.low_variance_ratio_threshold)
        )
        ...
        return {
            ...
            "variance_ratio_vs_baseline": float(baseline_ratio),
            "improbable": improbable,
        }

    def _effective_injection_thresholds(self) -> tuple[float, float, float]:
        """Compute adaptive ranges for low-delta and high-inconsistent regimes.

        Calcula umbrales adaptativos para regímenes de delta bajo y alta inconsistencia.
        """
        if not self.adaptive_thresholds_enabled:
            return 0.0, float(self.progressive_injection_threshold), float(self.high_inconsistent_threshold)

        historical_deltas = [float(event.impacted_total_votes) for event in self.events if event.impacted_total_votes > 0]
        historical_inconsistent = [float(snapshot.inconsistent_count) for snapshot in self.snapshots]

        if len(historical_deltas) < self.min_baseline_cycles or len(historical_inconsistent) < self.min_baseline_cycles:
            return 0.0, float(self.progressive_injection_threshold), float(self.high_inconsistent_threshold)

        low_delta_dynamic_max = self._percentile(historical_deltas, self.low_delta_percentile_range[1])
        high_inconsistent_dynamic_min = self._percentile(historical_inconsistent, self.high_inconsistent_percentile_range[0])

        # English/Español: keep sane guardrails while prioritizing adaptive behavior / mantener guardrails razonables priorizando comportamiento adaptativo.
        effective_low_delta_min = 0.0
        effective_low_delta_max = min(float(self.progressive_injection_threshold), low_delta_dynamic_max)
        effective_high_inconsistent_min = max(float(self.high_inconsistent_threshold), high_inconsistent_dynamic_min)
        return effective_low_delta_min, effective_low_delta_max, effective_high_inconsistent_min

    def _parse_range(self, raw_value: Any, fallback: tuple[float, float]) -> tuple[float, float]:
        """Parse and sanitize percentile range from runtime config.

        Parsea y sanea un rango de percentiles desde configuración en runtime.
        """
    ...
    def _percentile(self, values: list[float], q: float) -> float:
        """Return linear-interpolated percentile for small in-memory samples.

        Devuelve percentil con interpolación lineal para muestras pequeñas en memoria.
        """
        if not values:
            return 0.0
