...
def generate_dashboard_prompt() -> str:
    """Generate a bilingual design prompt for a world-class electoral audit dashboard."""
    prompt = """
    Diseña un dashboard web profesional de calidad mundial para 'Centinel: Centro de Vigilancia Electoral', un sistema pionero de auditoría electoral automatizada en Honduras/Centroamérica. El panel debe ser neutral, agnóstico a partidos políticos, enfocado exclusivamente en datos JSON del CNE (sin actas/mesas). Usa tema dark mode moderno con acentos verde/azul para confianza/integridad.

    Estructura principal:
    - Header: Logo C.E.N.T.I.N.E.L., login seguro, modo (Demo/Producción), timestamp de último audit (máx. cada 5min).
    - Sección Panorama en Tiempo Real: KPIs cards para Cobertura Activa (%), Deltas Críticos (anomalías estadísticas >2σ), Actas Inconsistentes (0), Hash Raíz (para verificación cripto), Estado Global (Stable/Alert con colores).
    - Panorama Nacional: Mapa interactivo de Honduras por departamento con heatmaps de integridad (usando distribuciones Benford/chi-cuadrado), stats globales verificables (integridad continua, deltas negativos/hora).
    - Timeline Interactivo: Gráfico de línea/barra con zoom/filtros para evolución de votos, anomalías detectadas (marcadas con tooltips matemáticos: e.g., 'Desviación: 3.2σ, p-value: 0.01').
    - Vistas Detalladas: Acordeones para Anomalías Detectadas (clasificadas: estadísticas, criptográficas, topológicas), Snapshots Recientes (tabla con hashes, deltas, verificación).
    - Reports & Export: Botones para generar PDF/CSV/JSON con evidencias matemáticas (ecuaciones, gráficos), watermark de inmutabilidad.

    Asegura:
    - Rigor matemático: Integra visuales de tests estadísticos (Benford, Kolmogorov-Smirnov, bayesiano), fórmulas visibles en tooltips.
    - Usabilidad: Responsive, accesible (WCAG), bilingüe (ES/EN), tooltips explicativos para expertos (matemáticos/políticos/OEA).
    - Seguridad: Indicadores de encriptación, no datos reales en demo (usa mocks de 2025).
    - Innovación: Elementos pioneros como alertas AI-driven, integración blockchain para hashes.

    Output: Describe el layout HTML/CSS o genera código React/Streamlit, o imagen mockup detallada.
    """
    return prompt.strip()
...
def create_pdf_charts(
    ...
    """Create chart buffers used by the PDF reporting pipeline."""
    if plt is None:
        return {}
...
def _generate_weasyprint_pdf(template_data: dict[str, Any]) -> bytes | None:
    """Render HTML template with Jinja2 and convert it to PDF using WeasyPrint."""
...
def _chart_to_base64(buf: io.BytesIO | None) -> str:
    """Convert a chart buffer to base64 so it can be embedded in HTML."""
    if buf is None:
        return ""
    buf.seek(0)
